...
#define os_dprintf(...) ll_printf("[OS]" __VA_ARGS__)
#else
#define os_dprintf(...)
#endif
 ...
 * @param[in] name Name of the new thread
 ...
 * @return Positive value if woken up before given ticks. The value
 * returned is amount of ticks left before the task was to be originally
 * scheduled to be woken up. So if sleep was for 10 ticks and the task is
 * woken up after 8 ticks then 2 will be returned.
 */
 ...
 * ticks. This is useful as functions like os_thread_sleep() accept only ticks
 * as input.
 *
 ...
 * @param[out] mhandle Pointer to a semaphore handle
 * @param[in] name Name of the semaphore
 * @param[in] maxcount  The maximum count value of this semaphore
 * @param[in] initcount The initial count on semaphore creation
 *
 * @return WM_SUCCESS on success
 * @return -WM_FAIL on error
 */
 ...
 *
 */
void os_rwlock_delele(os_rw_lock_t *lock);
 ...
 */
static inline int os_timer_activate(os_timer_t *timer_t)
{
	int ret;
 ...
 * @return -WM_FAIL on failure
 */
static inline int os_timer_is_active(os_timer_t *timer_t)
{
	int ret;
...
/** The block pool pointer */
typedef struct pool_info *os_block_pool_t;

/** Create block allocator memory pool
 *
 * This function creates a memory pool of a given block size that can be used by
 * the block allocator.
 *
 *  @param[out] pool_ptr Pointer to a block pool
 *  @param[in] name_ptr Name of the memory pool
 *  @param[in] block_size The pool's block size
 *  @param[in] pool_start A pointer to the caller allocated buffer. This buffer
 *  needs to be 4 byte aligned.
 *  @param[in] pool_size Total size of the buffer pointed by pool_start.
 *  @param[in] mutex_flag A flag to enable (set to 1) or disable( set to 0)
 *  mutex lock for the pool. If the pool is being accessed in the context of a
 *  single thread, you may disable mutex_flag, thus saving some time.
 *
 *  @return WM_SUCCESS if successful
 *  @return -WM_FAIL on failure
 */
static inline int os_block_pool_create(os_block_pool_t *pool_ptr,
				       char *name_ptr, unsigned long block_size,
				       void *pool_start,
				       unsigned long pool_size,
				       unsigned char mutex_flag)
{
	/* Return 0 on success, -1 on failure */
	*pool_ptr = ba_pool_create(block_size, pool_start, pool_size, mutex_flag);
	if (*pool_ptr == NULL)
		return -WM_FAIL;
	else
		return WM_SUCCESS;
...
/** Delete block allocator memory pool
 *
 * This function deletes a memory pool previously allocated by
 * os_block_pool_create().
 *
 * @param[in] pool_ptr Pointer to a block pool
 *
 * @return WM_SUCCESS if successful
 * @return -WM_FAIL on failure
 */
static inline int os_block_pool_delete(os_block_pool_t *pool_ptr)
{
	/* Return 0 on success, -1 on failure */
	int rv = ba_pool_delete(*pool_ptr);
	*pool_ptr = NULL;
	return rv;
}

/** Block Allocator: Allocate memory block
 *
 * This function allocates a memory block from a block pool. The block size to
 * allocate is mentioned while creating the block pool using
 * os_block_pool_create().
 *
 * @param[in] pool_ptr Pointer to the block pool
 * @param[out] block_ptr Pointer to a newly allocated block. This is NULL if
 * allocation is unsuccessful.
 * @param [in] wait_option This option is currently not supported.
 *
 * @return WM_SUCCESS if successful
 * @return -WM_FAIL on failure
 */
static inline int os_block_alloc(os_block_pool_t *pool_ptr, void **block_ptr,
				 unsigned long wait_option)
{
	/* Return NULL in case of failure, the allocation pointer if success */
	if (!(*block_ptr = ba_block_allocate(*pool_ptr)))
		return -WM_FAIL;
	else
		return WM_SUCCESS;
}

/** Block Allocator: Free memory block
 *
 * This function frees a memory block previously allocated using
 * os_block_alloc().
 *
 * @param [in] pool_ptr Pointer to a block pool
 * @param [in] block_ptr Pointer to the memory block to be freed
 *
 * @return WM_SUCCESS if successful
 * @return -WM_FAIL on failure
 */
static inline int os_block_free(os_block_pool_t *pool_ptr, void *block_ptr)
{
	/* Return 0 on success, -1 on failure */
	return ba_block_release(*pool_ptr, block_ptr);

}

/** Block Allocator: Get pool information
 *
 * @param[in] pool_ptr Pointer to a block pool
 * @param[out] str The buffer in which the pool information is to be obtained.
 * @param[in] length The size of the buffer str.
 *
 * @return WM_SUCCESS if successful
 * @return -WM_FAIL on failure
 */
static inline int os_block_pool_info_get_str(os_block_pool_t *pool_ptr,
					     char *str, unsigned long length)
{
	unsigned long bitmap, available_blocks, block_size;
	int ret;
	ret =
	    ba_block_pool_info_get(*pool_ptr, &bitmap, &available_blocks,
				   &block_size);
	if (!ret) {
		snprintf(str, length,
			 "\r\nBlock size: %ld\r\nAvailable blocks: %ld\r\nBitmap: 0x%02x\r\n",
			 block_size, available_blocks, (unsigned int) bitmap);
		return WM_SUCCESS;
	} else
		return -WM_FAIL;
}
