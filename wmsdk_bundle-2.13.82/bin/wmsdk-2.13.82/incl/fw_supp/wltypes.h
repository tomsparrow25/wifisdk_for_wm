...
#define MRVL_RATEID_BIT(rateId)     (1ULL << (rateId))
#else
typedef unsigned long       MRVL_RATEID_BITMAP_UNIT;
typedef unsigned long       MRVL_RATEID_BITMAP;
...
#if defined(DOT11N)
#ifdef DOT11AC
#define RATEID_MAX          RATEID_VHTMAX
#else
#define RATEID_MAX          RATEID_HTMAX
#endif
#else //11ABG only
#define RATEID_MAX          RATEID_OFDM54Mbps
#endif

#if defined(DOT11AC)
#define VHT_NUM_SUPPORT_MCS         10
#define VHT_NUM_SUPPORT_BW          3  //20,40,80
#ifdef STREAM_2x2
#define VHT_NUM_SUPPORT_SS          2
#else
#define VHT_NUM_SUPPORT_SS          1
#endif
#define VHT_NUM_MCS_SS              (VHT_NUM_SUPPORT_MCS * VHT_NUM_SUPPORT_SS)
#define VHT_NUM_RATE                (VHT_NUM_MCS_SS * VHT_NUM_SUPPORT_BW)

#define GET_VHT_MRVL_RATEID(m,s,b)  (RATEID_VHTMIN + (m) + \
                                     ((s) * VHT_NUM_SUPPORT_MCS) + \
                                     ((b) * VHT_NUM_SUPPORT_MCS * VHT_NUM_SUPPORT_SS))
#define VHT_MRVL_RATEID_GET_MCS(a)  (((a) - RATEID_VHTMIN) % VHT_NUM_SUPPORT_MCS)
#define VHT_MRVL_RATEID_GET_NSS(a)  ((((a) - RATEID_VHTMIN) % VHT_NUM_MCS_SS) / VHT_NUM_SUPPORT_MCS)
#define VHT_MRVL_RATEID_GET_BW(a)   (((a) - RATEID_VHTMIN) / VHT_NUM_MCS_SS)
...
#define RATE_BITMAP_IS_NOT_EMPTY(a)     (((a).bitmap[0] != 0) || \
                                         ((a).bitmap[1] != 0))
#define RATE_BITMAP_IS_EMPTY(a)         (((a).bitmap[0] == 0) && \
                                         ((a).bitmap[1] == 0))
									  
#define RATE_BITMAP_IS_SINGLE_RATE(a)   (((a).bitmap[0] && !((a).bitmap[0] & ((a).bitmap[0] - 1))) ^ \
                                         ((a).bitmap[1] && !((a).bitmap[1] & ((a).bitmap[1] - 1))))

#define RATE_BITMAP_IS_CCK_ONLY(a)       (((a).bitmap[1] == 0) && ((a).bitmap[0] <= RATEID_CCK_MASK))

///
#define RATE_BITMAP_ZERO(a)             {(a).bitmap[0] = 0; (a).bitmap[1] = 0;}
///
#define RATE_BITMAP_SET_RATE(a, rateId)    (((rateId) < NUM_RATE_BITMAP_UNIT) ? \
                                            ((a).bitmap[0] |= MRVL_RATEID_BIT(rateId)) : \
                                            ((a).bitmap[1] |= MRVL_RATEID_BIT(rateId - NUM_RATE_BITMAP_UNIT)))
#define RATE_BITMAP_FIND_RATE(a, rateId)   (((rateId) < NUM_RATE_BITMAP_UNIT) ? \
                                            ((a).bitmap[0] & MRVL_RATEID_BIT(rateId)) : \
                                            ((a).bitmap[1] & MRVL_RATEID_BIT(rateId - NUM_RATE_BITMAP_UNIT)))
///
#define RATE_BITMAP_SET_RATESET(a,b)       {(a).bitmap[0] |= (b).bitmap[0]; \
                                            (a).bitmap[1] |= (b).bitmap[1];}
#define RATE_BITMAP_CLR_RATESET(a,b)       {(a).bitmap[0] &= ~((b).bitmap[0]); \
                                            (a).bitmap[1] &= ~((b).bitmap[1]);}
#define RATE_BITMAP_COMMON_RATESET(a,b)    {(a).bitmap[0] &= (b).bitmap[0]; \
                                            (a).bitmap[1] &= (b).bitmap[1];}
///
#define RATE_BITMAP_SET_MASK(a, mask)        (((a).bitmap[0]) |= (mask))
#define RATE_BITMAP_CLR_MASK(a, mask)        (((a).bitmap[0]) &= ~(mask))
#define RATE_BITMAP_FIND_MASK(a, mask)       (((a).bitmap[0]) & (mask))

///
#define SHIFT_VHT_RATE0  (RATEID_VHTMIN)
#define MASK_VHT_RATE0   (~(MRVL_RATEID_BIT(RATEID_VHTMIN)-1))
#define SHIFT_VHT_RATE1  (64 - RATEID_VHTMIN)
#define MASK_VHT_RATE1   (MRVL_RATEID_BIT(RATEID_VHTMAX - 64 + 1) -1)


/* RATE_BITMAP_GET_HTLG and RATE_BITMAP_GET_VHT only work for up to 64 rates only */
#define RATE_BITMAP_GET_HTLG(a)              (((a).bitmap[0]) & ~MASK_VHT_RATE0)
#define RATE_BITMAP_GET_VHT(a)               ((((a).bitmap[0] >> SHIFT_VHT_RATE0) & (MRVL_RATEID_BIT(SHIFT_VHT_RATE1) - 1)) | \
                                              (((a).bitmap[1] & MASK_VHT_RATE1) << SHIFT_VHT_RATE1))

#define RATE_BITMAP_SET_VHTMASK(a, mask)     {(a).bitmap[0] |= (((mask)<<SHIFT_VHT_RATE0) & MASK_VHT_RATE0);\
                                              (a).bitmap[1] |= (((mask)>>SHIFT_VHT_RATE1) & MASK_VHT_RATE1);}

#define RATE_BITMAP_CLR_VHTMASK(a, mask)     {(a).bitmap[0] &= ~(((mask)<<SHIFT_VHT_RATE0) & MASK_VHT_RATE0);\
                                              (a).bitmap[1] &= ~(((mask)>>SHIFT_VHT_RATE1) & MASK_VHT_RATE1);}

#define RATE_BITMAP_FIND_VHTMASK(a, mask)    (((a).bitmap[0] & (((mask)<<SHIFT_VHT_RATE0) & MASK_VHT_RATE0)) || \
                                              ((a).bitmap[1] & (((mask)>>SHIFT_VHT_RATE1) & MASK_VHT_RATE1)))

#else //LG, HT1x1, HT2x2, VHT1x1


#define RATE_BITMAP_IS_EMPTY(a)            ((a) == 0)
#define RATE_BITMAP_IS_NOT_EMPTY(a)        ((a) != 0)
#define RATE_BITMAP_IS_SINGLE_RATE(a)      ((a) && !((a) & ((a) - 1)))
#define RATE_BITMAP_IS_CCK_ONLY(a)         ((a) <= RATEID_CCK_MASK)

#define RATE_BITMAP_ZERO(a)                ((a) = 0)

//TBD: compressed VHT bitmap
#define RATE_BITMAP_SET_RATE(a, rateId)    ((a) |= MRVL_RATEID_BIT(rateId))
#define RATE_BITMAP_FIND_RATE(a, rateId)   ((a) & MRVL_RATEID_BIT(rateId))

#define RATE_BITMAP_SET_RATESET(a,b)       ((a) |= (b))
#define RATE_BITMAP_CLR_RATESET(a,b)       ((a) &= ~(b))
#define RATE_BITMAP_COMMON_RATESET(a,b)    ((a) &= (b))

#define RATE_BITMAP_SET_MASK(a, mask)      ((a) |= (mask))
#define RATE_BITMAP_CLR_MASK(a, mask)      ((a) &= ~(mask))
#define RATE_BITMAP_FIND_MASK(a, mask)     ((a) & (mask))

#if defined(DOT11AC)
#define RATE_BITMAP_SET_VHTMASK(a, mask)     ((a) |= ((mask) << RATEID_VHTMIN))
#define RATE_BITMAP_CLR_VHTMASK(a, mask)     ((a) &= ~((mask) << RATEID_VHTMIN))
#define RATE_BITMAP_FIND_VHTMASK(a, mask)    ((a) & ((mask) << RATEID_VHTMIN))
#define RATE_BITMAP_GET_HTLG(a)              ((a) & (MRVL_RATEID_BIT(RATEID_VHTMIN)-1))
#define RATE_BITMAP_GET_VHT(a)               (((a) >> RATEID_VHTMIN) & (MRVL_RATEID_BIT(VHT_NUM_RATE) - 1))
#else
#define RATE_BITMAP_GET_HTLG(a)              (a)
#endif
