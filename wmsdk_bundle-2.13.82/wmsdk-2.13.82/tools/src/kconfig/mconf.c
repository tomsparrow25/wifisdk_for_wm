...
"Overview\n"
"--------\n"
"Some features may be built directly into WMSDK.\n"
...
"between different WMSDK configurations.\n"
"\n"
...
"------------------------------\n"
"If you prefer to have all of the WMSDK options listed in a single\n"
...
load_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep several different WMSDK\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than \n"
	"WMSDK's default, entering the name of the file here will allow you\n"
...
save_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep different WMSDK\n"
	"configurations available on a single machine.\n"
	"\n"
...
static void cprint_init(void)
{
	...
	indent = 0;
	child_count = 0;
	cprint("./tools/bin/Linux/lxdialog");
	cprint("--backtitle");
...
int main(int ac, char **av)
{
	...
	sym_calc_value(sym);
	sprintf(menu_backtitle, _("WMSDK Configuration"));
	...
	do {
		cprint_init();
		cprint("--yesno");
		cprint(_("Do you wish to save your new WMSDK configuration?"));
		cprint("5");
		cprint("60");
		stat = exec_conf();
	} while (stat < 0);
	...
	if (stat == 0) {
		if (conf_write(NULL)) {
			fprintf(stderr, _("\n\n"
				"Error during writing of the WMSDK configuration.\n"
				"Your WMSDK configuration changes were NOT saved."
				"\n\n"));
			return 1;
		}
		printf(_("\n\n"
			"*** End of WMSDK configuration.\n"
			"*** Execute 'make' to build WMSDK or try 'make help'."
			"\n\n"));
	} else {
		fprintf(stderr, _("\n\n"
			"Your WMSDK configuration changes were NOT saved."
