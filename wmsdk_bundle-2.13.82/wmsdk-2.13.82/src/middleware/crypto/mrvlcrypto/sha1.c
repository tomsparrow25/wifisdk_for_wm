...
static void pbkdf2_sha1_f(const char *passphrase, const char *ssid,
		size_t ssid_len, int iterations, unsigned int count, u8 *digest)
{
	...
	/* F(P, S, c, i) = U1 xor U2 xor ... Uc U1 = PRF(P, S || i)
	   U2 = PRF(P, U1)
	   Uc = PRF(P, Uc-1) */
	...
	for (i = 1; i < iterations; i++) {
		hmac_sha1((u8 *) passphrase, passphrase_len, tmp,
		...
		int iterations, u8 *buf, size_t buflen)
{
	...
	while (left > 0) {
		count++;
		pbkdf2_sha1_f(passphrase, ssid, ssid_len, iterations,
				count, digest);
