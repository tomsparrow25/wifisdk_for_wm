...
#define BN_S_MP_EXPTMOD_C       /* Note: #undef in tommath_superclass.h; this
                                   would require BN_MP_EXPTMOD_FAST_C instead */
...
#define XMALLOC  crypto_mem_malloc
#define XFREE    crypto_mem_free
#define XREALLOC crypto_mem_realloc
...
s_mp_add(mp_int * a, mp_int * b, mp_int * c)
{
    ...
    /* find sizes, we let |a| <= |b| which means we have to sort them.  "x"
       will point to the input with the most digits */
    if (a->used > b->used) {
        min = b->used;
        max = a->used;
        x = a;
...

    {
        ...
        /* now copy higher words if any, that is in A+B if A or B has more
           digits add those in */
        if (min != max) {
            for (; i < max; i++) {
                /* T[i] = X[i] + U */
...

    {
        ...
        for (i = 0; i < min; i++) {
            ...
            *tmpc = *tmpa++ - *tmpb++ - u;
            ...
            /* U = carry bit of T[i] Note this saves performing an AND
               operation since if a carry does occur it will propagate all the
               way to the MSB.  As a result a single shift is enough to get the 
               carry */
...
#endif
    {
        /* can we use the fast multiplier? The fast multiplier can be used if
           the output will have less than MP_WARRAY digits and the number of
...
mp_div_2d(mp_int * a, int b, mp_int * c, mp_int * d)
{
    ...
    if (D != 0) {
        ...
        for (x = c->used - 1; x >= 0; x--) {
            ...
            /* shift the current word and mix in the carry bits from the
               previous word */
            *tmpc = (*tmpc >> D) | (r << shift);
            --tmpc;
...
mp_rshd(mp_int * a, int b)
{
...

    {
        ...
        /* this is implemented as a sliding window where the window is b-digits 
           long and digits from the top of the window are copied to the bottom
           e.g.
...
mp_grow(mp_int * a, int size)
{
    ...
    if (a->alloc < size) {
        ...
        /* reallocate the array a->dp We store the return in a temporary
           variable in case the operation failed we don't want to overwrite the 
           dp member of a. */
...
mp_init_multi(mp_int * mp, ...)
{
    ...
    while (cur_arg != NULL) {
        if (mp_init(cur_arg) != MP_OKAY) {
            /* Oops - error! Back-track and mp_clear what we already succeeded
               in init-ing, then return error. */
            va_list clean_args;
...
mp_lshd(mp_int * a, int b)
{
...

    {
        ...
        /* much like mp_rshd this is implemented using a sliding window except
           the window goes the otherway around.  Copying from the bottom to the 
           top.  see bn_mp_rshd.c for more info. */
...
mp_div(mp_int * a, mp_int * b, mp_int * c, mp_int * d)
{
    ...
    for (i = n; i >= (t + 1); i--) {
        ...
        /* step 3.1 if xi == yt then set q{i-t-1} to b-1, otherwise set
           q{i-t-1} to (xi*b + x{i-1})/yt */
        if (x.dp[i] == y.dp[t]) {
    ...
    /* now q is the quotient and x is the remainder [which we have to
       normalize] */
...
s_mp_exptmod(mp_int * G, mp_int * X, mp_int * P, mp_int * Y, int redmode)
{
    ...
    /* create M table The M table contains powers of the base, e.g. M[x] = G**x 
       mod P The first half of the table is not computed though accept for M[0] 
       and M[1] */
    if ((err = mp_mod(G, P, &M[1])) != MP_OKAY) {
        goto LBL_MU;
    }

    /* compute the value at M[1<<(winsize-1)] by squaring M[1] (winsize-1)
       times */
    ...
    /* create upper table, that is M[x] = M[x-1] * M[1] (mod P) for x =
       (2**(winsize - 1) + 1) to (2**winsize - 1) */
    ...
    for (;;) {
        ...
        /* if the bit is zero and mode == 0 then we ignore it These represent
           the leading zero bits before the first 1 bit in the exponent.
           Technically this opt is not required but it does lower the # of
           trivial squaring/reductions used */
        if (mode == 0 && y == 0) {
            continue;
        }
...
s_mp_mul_digs(mp_int * a, mp_int * b, mp_int * c, int digs)
{
    ...
    for (ix = 0; ix < pa; ix++) {
        ...
        for (iy = 0; iy < pb; iy++) {
            ...
            r = ((mp_word) * tmpt) +
                ((mp_word) tmpx) * ((mp_word) * tmpy++) + ((mp_word) u);
 ...
 */
    ...
    /* rho = -1/m mod b */
    *rho =
        (unsigned long) (((mp_word) 1 << ((mp_word) DIGIT_BIT)) - x) & MP_MASK;
...
fast_mp_montgomery_reduce(mp_int * x, mp_int * n, mp_digit rho)
{
    ...
    /* first we have to get the digits of the input into an array of double
       precision words W[...] */
    {
        register mp_word *_W;
    ...
    /* now we proceed to zero successive digits from the least significant
       upwards */
    for (ix = 0; ix < n->used; ix++) {
        /* mu = ai * m' mod b We avoid a double precision multiplication (which 
           isn't required) by casting the value down to a mp_digit. Note this
           requires that W[ix-1] have the carry cleared (see after the inner
           loop) */
        register mp_digit mu;
        mu = (mp_digit) (((W[ix] & MP_MASK) * rho) & MP_MASK);

        /* a = a + mu * m * b**i This is computed in place and on the fly. The
           multiplication by b**i is handled by offseting which columns the
           results are added to. Note the comba method normally doesn't handle
           carries in the inner loop In this case we fix the carry from the
           previous column since the Montgomery reduction requires digits of the 
           result (so far) [see above] to work.  This is handled by fixing up
           one carry after the inner loop.  The carry fixups are done in order
           so after these loops the first m->used words of W[] have the carries
           fixed */
        {
            register int iy;
       ...
       those least significant digits we zeroed]. */
    {
        ...
        /* copy out, A = A/b**n The result is A/b**n but instead of converting
           from an array of mp_word to mp_digit than calling mp_rshd we just
           copy them in the right order */
...
mp_mul_2(mp_int * a, mp_int * b)
{
...

    {
        ...
        /* now zero any excess digits on the destination that we didn't write
           to */
        tmpb = b->dp + b->used;
        for (x = b->used; x < oldused; x++) {
            *tmpb++ = 0;
...
mp_montgomery_calc_normalization(mp_int * a, mp_int * b)
{
    ...
    if (b->used > 1) {
        if ((res =
             mp_2expt(a, (b->used - 1) * DIGIT_BIT + bits - 1)) != MP_OKAY) {
            return res;
        }
    } else {
        mp_set(a, 1);
...
mp_exptmod_fast(mp_int * G, mp_int * X, mp_int * P, mp_int * Y, int redmode)
{
    ...
    /* use a pointer to the reduction algorithm.  This allows us to use one of
       many reduction algorithms without modding the guts of the code with if
    ...
    if (redmode == 0) {
        /* fixup result if Montgomery reduction is used recall that any value
