...
#define WPA2_ERR_CONN_CLOSE	   -5   /* connection closed or epipe */
...
int MrvAES_Wrap(u8 *pPlainTxt, u32 PlainTxtLen, u8 *pCipTxt,
		u8 *pKEK, u32 KeyLen, u8 *IV);
int MrvAES_UnWrap(u8 *pCipTxt, u32 CipTxtLen, u8 *pPlainTxt,
		  u8 *pKEK, u32 KeyLen, u8 *IV);
...
extern int wps_uap_session_complete_handler(PWPS_INFO pwps_info,
					    WPS_DATA *wps_s);
extern void wps_shutdown();
extern u16 wps_probe_request_device_password_id_parser(u8 *message,
						       size_t size);
