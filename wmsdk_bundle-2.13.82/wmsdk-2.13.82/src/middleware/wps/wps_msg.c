...
static int wps_subele_authorized_mac_prepare(PWPS_INFO pwps_info, u8 *ptr)
{
	...
	while (auth_enrollee_count < MAX_AUTH_ENROLLEE_COUNT) {
		if (memcmp
		    (&pwps_info->auth_enrollee_mac_addr
		     [auth_enrollee_count * ETH_ALEN], wps_zero_mac,
		     ETH_ALEN)) {
			memcpy(ptr,
			       &pwps_info->auth_enrollee_mac_addr
			       [auth_enrollee_count * ETH_ALEN], ETH_ALEN);
			wps_hexdump(DEBUG_WLAN,
				    "Authorized Enrollee Mac address: ",
				    &pwps_info->auth_enrollee_mac_addr
				    [auth_enrollee_count * ETH_ALEN], ETH_ALEN);
...
wps_attribute_public_key_prepare(PWPS_INFO pwps_info, u8 type, u8 *ptr)
{
	...
	if (type == WPS_ENROLLEE) {
		...
		pwps_info->dh =
		    mrvl_dh_setup_key(pwps_info->enrollee.public_key,
				      SZ_PUBLIC_KEY,
		...
		mrvl_dh_free(pwps_info->dh);
		pwps_info->dh =
		    mrvl_dh_setup_key(pwps_info->registrar.public_key,
				      SZ_PUBLIC_KEY,
...
wps_attribute_primary_device_type_prepare(PWPS_INFO pwps_info, u8 *ptr)
{
	...
	primary_dev->sub_category_id =
	    htons(pwps_info->primary_dev_subcategory);
			   ...
			   u16 curr_msg_len, u8 *hmac_out)
{
	...
	mrvl_hmac_sha256(pwps_info->auth_key, SZ_AUTH_KEY,
...
int output_supp_config(PWPS_INFO pwps_info, int index, int nCred)
{
	...
	wps_printf(MSG_INFO, "Create config file for wpa_supplicant\r\n");
	wps_printf(MSG_INFO, "Auth Type: %x, Encryption Type: %x\r\n\r\n",
	...
	if (pCredential->auth_type == AUTHENTICATION_TYPE_WPAPSK ||
		...
		memcpy(network.name, pCredential->ssid,
		       pCredential->ssid_length);
		memcpy(network.ssid, pCredential->ssid,
		       pCredential->ssid_length);
#ifdef CONFIG_P2P
		...
		for (i = 0; i < nCred; i++) {
			if (nCred == 2)
				pCredential =
				    (CREDENTIAL_DATA *) &en_reg->cred_data[i];
			else
				pCredential =
				    (CREDENTIAL_DATA *) &
				    en_reg->cred_data[index];
		...
		memcpy(network.security.psk, pCredential->network_key,
		       pCredential->network_key_len);

		snprintf(psk_str, pCredential->network_key_len + 1, "%s",
				pCredential->network_key);

		if (pCredential->network_key_len == 64) {
			WPS_LOG("network = {ssid=\"%s\", proto=%s, key_mgmt=%s,"
			" pairwise=%s, ", pCredential->ssid, proto_str,
			key_mgmt_str, pairwise_str);
			WPS_LOG("psk=%s}", psk_str);
		} else {
			WPS_LOG("network = {ssid=\"%s\", proto=%s, key_mgmt=%s,"
			" pairwise=%s, psk=\"%s\"}", pCredential->ssid,
			proto_str, key_mgmt_str, pairwise_str, psk_str);
		}
		...
		memcpy(network.ssid, pCredential->ssid,
		       pCredential->ssid_length);
		memcpy(network.bssid, wps_network.bssid,
		...
		if (pCredential->encry_type == ENCRYPTION_TYPE_WEP) {
			...
			if (pCredential->wep_tx_key_index) {
				/* WEP Key Index of wpa_supplicant
				   is "0" based index */
				sprintf(wep_key_index_str, "%s%d=", "wep_key",
			 ...
			 */
				...
				sprintf(wep_key_str, "%02x%02x%02x%02x%02x",
					pCredential->network_key[0],
					pCredential->network_key[1],
					pCredential->network_key[2],
					pCredential->network_key[3],
					pCredential->network_key[4]);
			} else if (pCredential->network_key_len == 13) {
				sprintf(wep_key_str, "%02x%02x%02x%02x%02x%02x"
					...
					pCredential->network_key[3],
					pCredential->network_key[4],
					pCredential->network_key[5],
					pCredential->network_key[6],
					pCredential->network_key[7],
					pCredential->network_key[8],
					pCredential->network_key[9],
					pCredential->network_key[10],
					pCredential->network_key[11],
		...
		wmprintf("\r\n");
		if (pCredential->auth_type ==
			    AUTHENTICATION_TYPE_SHARED)
			WPS_LOG("network = {ssid=\"%s\", key_mgmt=NONE, %s=%s, "
				"%s, auth_alg=SHARED}\r\n",
				pCredential->ssid, wep_key_index_str,
				wep_key_str, wep_tx_keyidx_str);
		else
			WPS_LOG("network = {ssid=\"%s\", key_mgmt=NONE}\r\n",
				pCredential->ssid);
	} else {
		WPS_LOG("ERROR: Unexpected Auth Type:: %0x !", pCredential->auth_type);
		return WPS_STATUS_FAIL;
	}
...
int wps_write_credentials(PWPS_INFO pwps_info)
{
	...
	for (i = 0; i < num_cred; i++) {
		if ((pwps_info->registrar.cred_data[i].rf_bands == 1) &&
			(pwps_info->registrar.cred_data[i].auth_type >=
			pwps_info->registrar.cred_data[index_g].auth_type))
				index_g = i;

#ifdef CONFIG_5GHz_SUPPORT
		if ((pwps_info->registrar.cred_data[i].rf_bands == 2) &&
			(pwps_info->registrar.cred_data[i].auth_type >=
			pwps_info->registrar.cred_data[index_a].auth_type))
				index_a = i;
#endif /* CONFIG_5GHz_SUPPORT */
	}
...
{
	PEAP_TLS_FRAME_HEADER peap_wps_frame;
	u8 *ptr;
	u16 length;
	int ret;
	...
	if (gpwps_info_initialized) {
		...
		peap_wps_frame = (PEAP_TLS_FRAME_HEADER) gpwps_info->buffer;
		ptr = gpwps_info->buffer;
		peap_wps_frame->code = EAP_RESPONSE;
		peap_wps_frame->identifier = gpwps_info->peer_id;
		peap_wps_frame->type = EAP_TYPE_TLS;
#ifdef CONFIG_FREE_RADIUS
		if (len > 600 || (flag && more_frags < 4)) {
			flag = true;
			more_frags++;
			peap_wps_frame->flags = 0x40;
		} else
#endif
			peap_wps_frame->flags = 0x00;
		peap_wps_frame->length = 6;

		if ((buf != NULL) && len) {
			ptr += peap_wps_frame->length;
			memcpy(ptr, buf, len);
			peap_wps_frame->length += len;
		}
		...
		/* call eapol tx function */
		ret = wps_eapol_txPacket((u8 *) gpwps_info->buffer,
					(size_t) length);
...
int wps_eap_response_identity_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
	...
	if (memcmp(buf, wps_registrar_identity,
		...
		if ((wps_s->bss_type == BSS_TYPE_UAP &&
			...
			WPS_LOG("Switching Role to Registrar...");
			memcpy(pwps_info->enrollee.mac_address,
			       pwps_info->registrar.mac_address, ETH_ALEN);
			memcpy(pwps_info->registrar.mac_address,
			       wps_s->my_mac_addr, ETH_ALEN);
		...
		wps_printf(DEBUG_WPS_MSG,
			   "Unknown EAP Response Identity. Quitting");
		LEAVE();
		return WPS_STATUS_FAIL;
	}
...
int wps_eap_M1_frame_prepare(PWPS_INFO pwps_info)
{
	...
	wps_printf(DEBUG_WPS_MSG, "M1 Auth type flags %4x", msg_value_short);
	...
	wps_printf(DEBUG_WPS_MSG, "M1 Encry type flags %4x", msg_value_short);
	memcpy(ptr, &msg_value_short, 2);
	message_length += SZ_TLV_HEADER + ptlv->length;
	ptr += ptlv->length;
	ptlv->length = htons(ptlv->length);
	...
	wps_printf(DEBUG_WPS_MSG, "Enrollee DevID = 0x%x", msg_value_short);
	memcpy(ptr, &msg_value_short, 2);
	message_length += SZ_TLV_HEADER + ptlv->length;
	ptr += ptlv->length;
	ptlv->length = htons(ptlv->length);
	...
	while ((void *)data < (void *)data_end) {
		...
		case SC_AuthorizedMACs:
			...
			while (auth_tlv_len > 0) {
				...
				if (memcmp
				    (wps_s->my_mac_addr, &data[2] + offset,
				     ETH_ALEN) == 0
				    || memcmp(wps_broadcast_mac,
					      &data[2] + offset,
					      ETH_ALEN) == 0) {
					pwps_info->enrollee_in_authorized_mac =
					    WPS_SET;
					wps_printf(DEBUG_WPS_MSG,
...
wps_process_attrib_vendor_ext(PWPS_INFO pwps_info, u8 *data, u16 tlv_length)
{
	...
	if (!memcmp(data, wfa_vendor_ext_id, WPS_VENDOR_EXT_ID_LEN)) {
		ret =
		    wps_process_wfa_vendor_ext(pwps_info,
					       data + WPS_VENDOR_EXT_ID_LEN,
					       tlv_length -
					       WPS_VENDOR_EXT_ID_LEN);
	}
...
int wps_eap_M1_frame_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
	...
	while ((void *)ptlv < (void *)plast_byte) {
		...
		case SC_Vendor_Extension:
			wps_process_attrib_vendor_ext(pwps_info, data,
						      tlv_length);
			if (pwps_info->enrollee.version2 > WPS_VERSION) {
				wps_printf(MSG_WARNING,
					   "(M1) Unknown WPS Version = 0x%x",
					   pwps_info->enrollee.version2);
			}
			break;
		case SC_Message_Type:
			wps_printf(DEBUG_WPS_MSG, "Message_Type = 0x%x", *data);
			if (*data != WPS_M1)
				return WPS_STATUS_FAIL;
			break;
		case SC_UUID_E:
		...
		case SC_MAC_Address:
			if (memcmp
			    (pwps_info->enrollee.mac_address, data,
			     tlv_length) != 0) {
				wps_printf(MSG_WARNING,
			   "Warning : Enrollee MAC Address Not Matched !");
				wps_hexdump(DEBUG_WPS_MSG, "SC_MAC_Address",
					    data, tlv_length);
				memcpy(pwps_info->enrollee.mac_address, data,
				       tlv_length);
			} else {
				wps_printf(DEBUG_WPS_MSG,
		...
		case SC_Enrollee_Nonce:
			...
			break;
		case SC_Public_Key:
			memcpy(pwps_info->enrollee.public_key, data,
			       tlv_length);
			break;
		case SC_Authentication_Type_Flags:
			memcpy(&pwps_info->enrollee.auth_type_flag, data,
			       tlv_length);
		...
		case SC_Encryption_Type_Flags:
			memcpy(&pwps_info->enrollee.encry_type_flag, data,
			       tlv_length);
		...
		case SC_Connection_Type_Flags:
			memcpy(&pwps_info->enrollee.connection_type_flag, data,
			       tlv_length);
			break;
		case SC_Config_Methods:
			memcpy(&pwps_info->enrollee.config_methods, data,
			       tlv_length);
			pwps_info->enrollee.config_methods =
			    ntohs(pwps_info->enrollee.config_methods);
			break;
		case SC_Simple_Config_State:
			memcpy(&pwps_info->enrollee.simple_config_state, data,
			       tlv_length);
			break;
		case SC_Manufacturer:
			memcpy(pwps_info->enrollee.manufacture, data,
			       tlv_length);
			pwps_info->enrollee.manufacture_length = tlv_length;
			break;
		case SC_Model_Name:
			memcpy(pwps_info->enrollee.model_name, data,
			       tlv_length);
			pwps_info->enrollee.model_name_length = tlv_length;
			break;
		case SC_Model_Number:
			memcpy(pwps_info->enrollee.model_number, data,
			       tlv_length);
			pwps_info->enrollee.model_number_length = tlv_length;
			break;
		case SC_Serial_Number:
			memcpy(pwps_info->enrollee.serial_number, data,
			       tlv_length);
			pwps_info->enrollee.serial_number_length = tlv_length;
			break;
		case SC_Primary_Device_Type:
			memcpy(pwps_info->enrollee.primary_device_type, data,
			       tlv_length);
			break;
		case SC_Device_Name:
			memcpy(pwps_info->enrollee.device_name, data,
			       tlv_length);
		...
		case SC_Association_State:
			memcpy(&pwps_info->enrollee.association_state, data,
			       tlv_length);
			break;
		case SC_Device_Password_ID:
			memcpy(&pwps_info->enrollee.device_password_id, data,
			       tlv_length);
			pwps_info->enrollee.device_password_id =
			    ntohs(pwps_info->enrollee.device_password_id);
			break;
		case SC_Configuration_Error:
			memcpy(&pwps_info->enrollee.configuration_error, data,
			       tlv_length);
			pwps_info->enrollee.configuration_error =
			    ntohs(pwps_info->enrollee.configuration_error);
			wps_printf(DEBUG_WPS_MSG,
				   "SC_Configuration_Error(M1): %d",
				   pwps_info->enrollee.configuration_error);
			break;
		case SC_OS_Version:
			memcpy(&pwps_info->enrollee.os_version, data,
			       tlv_length);
 ...
 *				on fail
 */
static int wps_authenticator_validate(PWPS_INFO pwps_info,
...
static int wps_derive_kdk(PWPS_INFO pwps_info)
{
	...
	wps_printf(DEBUG_WPS_MSG, "Agree Key Length = %d", AgreedKeyLen);
	...
	if (!mrvl_sha256_vector(1, msg, &AgreedKeyLen, DHKey, sizeof(DHKey))) {
		wps_printf(MSG_ERROR, "SHA256 failed!\n");
		return WPS_STATUS_FAIL;
	}
	...
	mrvl_hmac_sha256(DHKey, SHA256_DIGEST_SIZE,
...
int wps_eap_M2_frame_prepare(PWPS_INFO pwps_info)
{
	...
	ptr += offset;
	offset =
	    wps_attribute_public_key_prepare(pwps_info, WPS_REGISTRAR, ptr);
	message_length += offset;
	...
	wps_printf(DEBUG_WPS_MSG, "M2 Auth type flags %4x", msg_value_short);
	memcpy(ptr, &msg_value_short, 2);
	message_length += SZ_TLV_HEADER + ptlv->length;
	ptr += ptlv->length;
	ptlv->length = htons(ptlv->length);
	...
	wps_printf(DEBUG_WPS_MSG, "M2 Encry type flags %4x", msg_value_short);
	memcpy(ptr, &msg_value_short, 2);
	message_length += SZ_TLV_HEADER + ptlv->length;
	ptr += ptlv->length;
	ptlv->length = htons(ptlv->length);
	...
	ptr += SZ_TLV_HEADER;
	msg_value_short =
	    htons(pwps_info->registrar.updated_device_password_id);
	memcpy(ptr, &msg_value_short, 2);
	message_length += SZ_TLV_HEADER + ptlv->length;
	ptr += ptlv->length;
	ptlv->length = htons(ptlv->length);
	...
	offset =
	    wps_attribute_authenticator_prepare(pwps_info, message_len, ptr);
	message_length += offset;
	ptr += offset;
...
int wps_eap_M2_frame_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
	...
	while ((void *)ptlv < (void *)plast_byte) {
		...
		case SC_UUID_R:
			...
			break;
		case SC_Public_Key:
			memcpy(pwps_info->registrar.public_key, data,
			       tlv_length);
			break;
		case SC_Authenticator:
			memcpy(pwps_info->registrar.authenticator, data,
			       tlv_length);
			break;
		case SC_Authentication_Type_Flags:
			memcpy(&pwps_info->registrar.auth_type_flag, data,
			       tlv_length);
			pwps_info->registrar.auth_type_flag =
			    ntohs(pwps_info->registrar.auth_type_flag);
			wps_printf(DEBUG_WPS_MSG,
				   "Authentication_Type_Flags(M2) = 0x%04x",
				   pwps_info->registrar.auth_type_flag);
			if (!wps_process_authentication_type_flags(pwps_info)) {
				wps_printf(MSG_ERROR,
					   "Authentication type flag from "
			...
			break;
		case SC_Encryption_Type_Flags:
			memcpy(&pwps_info->registrar.encry_type_flag, data,
			       tlv_length);
			pwps_info->registrar.encry_type_flag =
			    ntohs(pwps_info->registrar.encry_type_flag);
			wps_printf(DEBUG_WPS_MSG,
				   "Encryption_Type_Flags(M2) = 0x%04x",
				   pwps_info->registrar.encry_type_flag);
			if (!wps_process_encryption_type_flags(pwps_info)) {
				wps_printf(MSG_ERROR,
					   "Encryption type flag from Enrollee"
		...
		case SC_Connection_Type_Flags:
			...
			break;
		case SC_Config_Methods:
			memcpy(&pwps_info->registrar.config_methods, data,
			       tlv_length);
			...
			break;
		case SC_Manufacturer:
			memcpy(pwps_info->registrar.manufacture, data,
			       tlv_length);
			pwps_info->registrar.manufacture_length = tlv_length;
			break;
		case SC_Model_Name:
			memcpy(pwps_info->registrar.model_name, data,
			       tlv_length);
			pwps_info->registrar.model_name_length = tlv_length;
			break;
		case SC_Model_Number:
			memcpy(pwps_info->registrar.model_number, data,
			       tlv_length);
			pwps_info->registrar.model_number_length = tlv_length;
			break;
		case SC_Serial_Number:
			memcpy(pwps_info->registrar.serial_number, data,
			       tlv_length);
			pwps_info->registrar.serial_number_length = tlv_length;
			break;
		case SC_Primary_Device_Type:
			memcpy(pwps_info->registrar.primary_device_type, data,
			       tlv_length);
			break;
		case SC_Device_Name:
			memcpy(pwps_info->registrar.device_name, data,
			       tlv_length);
			pwps_info->registrar.device_name_length = tlv_length;
			break;
		case SC_RF_Band:
			memcpy(&pwps_info->registrar.rf_bands, data,
			       tlv_length);
			break;
		case SC_Association_State:
			memcpy(&pwps_info->registrar.association_state, data,
			       tlv_length);
			break;
		case SC_Configuration_Error:
			memcpy(&pwps_info->registrar.configuration_error, data,
			       tlv_length);
			pwps_info->registrar.configuration_error =
			    ntohs(pwps_info->registrar.configuration_error);
			wps_printf(DEBUG_WPS_MSG,
				   "SC_Configuration_Error(M2): %d",
				   pwps_info->registrar.configuration_error);
			break;
		case SC_Device_Password_ID:
			memcpy(&pwps_info->registrar.device_password_id, data,
			       tlv_length);
			break;
		case SC_OS_Version:
			memcpy(&pwps_info->registrar.os_version, data,
			       tlv_length);
	...
	/* Derive Keys */
	wps_derive_kdk(pwps_info);
	if (wps_authenticator_validate
	    (pwps_info, wps_frm_body, wps_frm_body_len)
	    != WPS_STATUS_SUCCESS) {
		return WPS_STATUS_FAIL;
	}
...
int wps_eap_M3_frame_prepare(PWPS_INFO pwps_info)
{
	...
	offset += PSK_LEN;
	memcpy(HashArgu + offset, pwps_info->enrollee.public_key,
	       WPS_PUB_KEY_LEN);
	offset += WPS_PUB_KEY_LEN;
	memcpy(HashArgu + offset, pwps_info->registrar.public_key,
	       WPS_PUB_KEY_LEN);
	offset += WPS_PUB_KEY_LEN;
	mrvl_hmac_sha256(pwps_info->auth_key, SZ_AUTH_KEY,
			 HashArgu, offset, HASH, SHA256_DIGEST_SIZE);
...
int wps_eap_M3_frame_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
	...
	while ((void *)ptlv < (void *)plast_byte) {
		...
		case SC_Message_Type:
			...
			break;
		case SC_Registrar_Nonce:
			if (memcmp(pwps_info->registrar.nonce, data, tlv_length)
			    != 0) {
				wps_printf(DEBUG_WPS_MSG,
			...
			wps_printf(DEBUG_WPS_MSG,
				   "@M3_process: SC_Registrar_Nonce OK");
			memcpy(pwps_info->registrar.nonce, data, tlv_length);
			break;
		case SC_E_Hash1:
		...
		case SC_E_Hash2:
			...
			break;
		case SC_Authenticator:
			memcpy(pwps_info->enrollee.authenticator, data,
			       tlv_length);
			break;
		default:
			wps_printf(DEBUG_WPS_MSG, "Unknown message_Type = 0x%x",
...
int wps_eap_M4_frame_prepare(PWPS_INFO pwps_info)
{
	...
	mrvl_aes_wrap(EncryData, pEncry - EncryData, EncryAesOutput,
...
int wps_eap_M4_frame_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
	...
	while ((void *)ptlv < (void *)plast_byte) {
		...
		case SC_Message_Type:
			...
			break;
		case SC_Enrollee_Nonce:
			if (memcmp(pwps_info->enrollee.nonce, data, tlv_length)
			    != 0) {
				wps_printf(DEBUG_WPS_MSG,
		...
		case SC_Encrypted_Settings:
			{
				...
				PTLV_DATA_HEADER pes =
				    (PTLV_DATA_HEADER) (pwps_info->
						registrar.wrap_raw_data);
				memset(pwps_info->registrar.wrap_raw_data, 0x00,
				       sizeof(pwps_info->
					      registrar.wrap_raw_data));
				mrvl_aes_unwrap(pwps_info->
						registrar.encrypted_data,
						pwps_info->
						registrar.encrypted_data_len,
						pwps_info->
						registrar.wrap_raw_data,
				...
				while (((u8 *) pes -
					pwps_info->registrar.wrap_raw_data) <
				       (pwps_info->
					registrar.encrypted_data_len - 16)) {
					...
					case SC_R_SNonce1:
						memcpy(pwps_info->
						       registrar.r_s1, esdata,
						       pes->length);
						break;
					case SC_Key_Wrap_Authenticator:
						memcpy
					    (pwps_info->key_wrap_authenticator,
						     esdata, pes->length);
						break;
					}
					elen = pes->length + SZ_TLV_HEADER;
					pes =
					    (PTLV_DATA_HEADER) ((u8 *) pes +
								elen);
				}	/* while */
...
int wps_eap_M5_frame_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
	...
	while ((void *)ptlv < (void *)plast_byte) {
		...
		case SC_Registrar_Nonce:
			...
			wps_printf(DEBUG_WPS_MSG,
				   "@M5_process: SC_Registrar_Nonce OK");
			memcpy(pwps_info->registrar.nonce, data, tlv_length);
			break;
		case SC_Encrypted_Settings:
			{
				...
				pwps_info->enrollee.encrypted_data_len =
				    tlv_length - 16;
				memcpy(pwps_info->enrollee.encrypted_data,
				       data + 16,
				       pwps_info->enrollee.encrypted_data_len);

				PTLV_DATA_HEADER pes =
				    (PTLV_DATA_HEADER) (pwps_info->
							enrollee.wrap_raw_data);
				memset(pwps_info->enrollee.wrap_raw_data, 0x00,
				       sizeof(pwps_info->
					      enrollee.wrap_raw_data));
				mrvl_aes_unwrap(pwps_info->
						enrollee.encrypted_data,
						pwps_info->
						enrollee.encrypted_data_len,
						pwps_info->
						enrollee.wrap_raw_data,
						pwps_info->key_wrap_key, 16,
				...
				while (((u8 *) pes -
					...
					switch (estype) {
					case SC_E_SNonce1:
						memcpy(pwps_info->enrollee.e_s1,
						       esdata, pes->length);
						break;
					case SC_Key_Wrap_Authenticator:
						memcpy
...
int wps_add_ap_settings_to_registrar(PWPS_INFO pwps_info, u8 *pEncry)
{
	...
	wps_printf(DEBUG_WPS_MSG, "AP Setting len: %d", ap_setting_len);
	return ap_setting_len;
}
#endif

#ifndef CONFIG_CYASSL
int
		...
		u8 *pKEK, u32 KeyLen, u8 * IV)
{
	...
	if (TextLen % 32 == 0) {
		wps_hexdump(DEBUG_WPS_MSG, "M8_preEncry", (u8 *) pPlainTxt,
			    TextLen);
...
int wps_eap_M7_frame_prepare(PWPS_INFO pwps_info)
{
	...
	memset(KWA, 0, sizeof(KWA));
	mrvl_hmac_sha256(pwps_info->auth_key, SZ_AUTH_KEY,
			 EncryData, pEncry - EncryData, KWA,
			 SHA256_DIGEST_SIZE);
	...
	wps_hexdump(DEBUG_WPS_MSG, "M7_preEncry", (u8 *) EncryData,
		    pEncry - EncryData);
	enc_setting_len =
	    mrvl_aes_wrap_ext(EncryData, pEncry - EncryData, EncryAesOutput,
			      pwps_info->key_wrap_key, 16,
			      pwps_info->enrollee.IV);
...
int wps_eap_M7_frame_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
	...
	while ((void *)ptlv < (void *)plast_byte) {
		...
		case SC_Registrar_Nonce:
			...
			wps_printf(DEBUG_WPS_MSG,
				   "@M7_process: SC_Registrar_Nonce OK");
			memcpy(pwps_info->registrar.nonce, data, tlv_length);
			break;
		case SC_Encrypted_Settings:
			{
				...
				CREDENTIAL_DATA *pCred =
				    (CREDENTIAL_DATA *) &pwps_info->
				    enrollee.cred_data[0];
				...
				while (((u8 *) pes -
					...
					switch (estype) {
					case SC_E_SNonce2:
						memcpy(pwps_info->enrollee.e_s2,
						       esdata, pes->length);
						break;
					case SC_Key_Wrap_Authenticator:
						memcpy
					...
					case SC_Authentication_Type:
						...
						memcpy(&pCred->auth_type,
						       esdata, pes->length);
						pCred->auth_type =
						    ntohs(pCred->auth_type);
						wps_printf(DEBUG_WPS_MSG,
					...
					case SC_Network_Key:
						wps_hexdump(DEBUG_WPS_MSG,
							    "SC_Network_Key",
							    (u8 *) esdata,
							    pes->length);
						memcpy(pCred->network_key,
						       esdata, pes->length);
...
int wps_eap_M8_frame_prepare(PWPS_INFO pwps_info)
{
	...
	*pEncry = 0x1;
	...
	if (!pwps_info->enrollee.version2 &&
		...
		msg_value_short =
		    htons(pwps_info->registrar.cred_data[0].auth_type);
	}
	memcpy(pEncry, &msg_value_short, 2);
	pEncry += ptlvC->length;
	enc_setting_len += (SZ_TLV_HEADER + ptlvC->length);
	...
	if (pwps_info->enrollee.version2) {
		msg_value_short =
		    htons(pwps_info->registrar.cred_data[0].encry_type);
	} else if (pwps_info->registrar.cred_data[0].encry_type ==
		...
		if (pwps_info->enrollee.encry_type_flag & ENCRYPTION_TYPE_AES)
			msg_value_short = htons(ENCRYPTION_TYPE_AES);
			 ...
			 enrollee.encry_type_flag & ENCRYPTION_TYPE_TKIP)
	...
	if (datalength % 32 == 0) {
		wps_hexdump(DEBUG_WPS_MSG, "M8_preEncry", (u8 *) EncryData,
			    datalength);
		memset(EncryAesOutput, 0, sizeof(EncryAesOutput));
		mrvl_aes_wrap(EncryData, datalength, EncryAesOutput,
			      pwps_info->key_wrap_key, 16,
			      pwps_info->registrar.IV);
...
wps_process_ap_settings_from_registrar(PWPS_INFO pwps_info, u8 *ptr, u32 len)
{
	...
	if (pwps_info->enrollee.version >= WPS_VERSION_2DOT0) {
		...
		case ENCRYPTION_TYPE_WEP:
			WPS_LOG("Invalid WSC2.0 credential. Encryption WEP!");
			pwps_info->read_ap_config_only = WPS_SET;
			ret = WPS_STATUS_FAIL;
			break;
		...
		case SC_Encrypted_Settings:
			{
				...
				wps_hexdump(DEBUG_WPS_MSG,
					    "M8_process Key Wrap Key",
					    (u8 *) pwps_info->key_wrap_key,
					    sizeof(pwps_info->key_wrap_key));

				wps_hexdump(DEBUG_WPS_MSG, "M8_Decry i/p",
					    (u8 *) pwps_info->
					    registrar.encrypted_data,
					    pwps_info->
					    registrar.encrypted_data_len);
				...
				pwps_info->registrar.encrypted_data_len =
				    mrvl_aes_unwrap_ext(pwps_info->
						registrar.encrypted_data,
							pwps_info->
						registrar.encrypted_data_len,
							pwps_info->
							registrar.wrap_raw_data,
							pwps_info->key_wrap_key,
							16,
							pwps_info->
							registrar.IV);

				wps_hexdump(DEBUG_WPS_MSG,
					    "M8_process Wrap Raw Data",
					    (u8 *) pwps_info->
					    registrar.wrap_raw_data,
					    pwps_info->
					    registrar.encrypted_data_len);
				...
				if (wps_s->bss_type == BSS_TYPE_UAP) {
					ret =
				    wps_process_ap_settings_from_registrar
					    (pwps_info, (u8 *) pes,
					     (pwps_info->registrar.
					      encrypted_data_len));
				...
				while (((u8 *) pes -
					...
					case SC_Credential:
						...
						case SC_Authentication_Type:
							wps_printf(
				DEBUG_WPS_MSG, "SC_Authentication_Type");
							memcpy
							    (&pCred->auth_type,
							     credata,
							     pcre->length);
						...
						case SC_Encryption_Type:
							wps_printf(
					DEBUG_WPS_MSG, "SC_Encryption_Type");
							memcpy
							    (&pCred->encry_type,
								     credata,
							     pcre->length);
						...
						case SC_Network_Key_Index:
							wps_printf(
					DEBUG_WPS_MSG, "SC_Network_Key_Index");
							memcpy
						    (&pCred->network_key_index,
							     credata,
						     pcre->length);
						...
						case SC_WEP_Transmit_Key:
							wps_printf(
					DEBUG_WPS_MSG, "SC_WEP_Transmit_Key");
							memcpy
						    (&pCred->wep_tx_key_index,
							     credata,
						     pcre->length);
							break;
						case SC_Network_Key:
							wps_printf(
					DEBUG_WPS_MSG, "SC_Network_Key");
							wps_hexdump
							    (DEBUG_WPS_MSG,
							     "SC_Network_Key",
							     (u8 *)
				...
				if (pwps_info->enrollee.version >=
				    WPS_VERSION_2DOT0) {
					for (cred_ind = 0; cred_ind < num_cred;
					     cred_ind++) {
						CREDENTIAL_DATA *pCred;
...
int wps_done_message_prepare(PWPS_INFO pwps_info)
{
	...
	/* call eapol tx function */
	ret =
	    wps_eapol_txPacket((u8 *) pwps_info->buffer,
...
int wps_done_message_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
	...
	while ((void *)ptlv < (void *)plast_byte) {
		...
		switch (ptlv->type) {
		case SC_Version:
			memcpy(&pwps_info->enrollee.version, data,
			       ptlv->length);
			break;
		case SC_Vendor_Extension:
			wps_process_attrib_vendor_ext(pwps_info, data,
	...
	while ((void *)ptlv < (void *)plast_byte) {
		...
		case SC_Version:
			if (pwps_info->role == WPS_ENROLLEE
				...
				memcpy(&pwps_info->registrar.version, data,
				       ptlv->length);
			} else if (pwps_info->role == WPS_REGISTRAR
				   || (IS_DISCOVERY_REGISTRAR(pwps_info))
			    ) {
...
short wps_selected_registrar_config_method_prepare(PWPS_INFO pwps_info)
{
	...
	if (en_reg->device_password_id == DEVICE_PASSWORD_PUSH_BUTTON)
		...
		msg_value_short = config_methods &
				(CONFIG_METHOD_KEYPAD | CONFIG_METHOD_DISPLAY);
#if 0
...
int wps_ap_beacon_prepare(u8 selectedRegistrar, u8 *ptr)
{
	...
	if (selectedRegistrar == 1) {
		...
		*ptr = selectedRegistrar;
		...
		ptr += SZ_TLV_HEADER;
		msg_value_short =
		    htons(wps_selected_registrar_config_method_prepare
			  (pwps_info));
		memcpy(ptr, &msg_value_short, 2);
		message_length += SZ_TLV_HEADER + ptlv->length;
		ptr += ptlv->length;
...
int wps_ap_probe_response_prepare(u8 selectedRegistrar, u8 *ptr)
{
	...
	ptr += SZ_TLV_HEADER;
	{
		if (en_reg->device_password_id == DEVICE_PASSWORD_PUSH_BUTTON)
			msg_value_short =
				htons(pwps_info->registrar.config_methods &
