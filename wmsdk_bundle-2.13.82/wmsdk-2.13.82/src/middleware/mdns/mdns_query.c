...
extern struct mdns_service_config config_g[MDNS_MAX_SERVICE_CONFIG];
...
int query_send_ctrl_msg(struct query_ctrl_msg *msg, uint16_t port)
{
	...
	FD_SET(s, &fds);
	SET_TIMEOUT(&t, 2000);
	ret = net_select(s + 1, &fds, NULL, NULL, &t);
	if (ret == -1) {
...
int query_launch()
{
	...
	if (query_thread == NULL)
		return -WM_E_MDNS_FQUERY_THREAD;
	return WM_SUCCESS;
}
...
static int signal_and_wait_for_query_halt()
{
	...
	return query_enabled ? -WM_FAIL : WM_SUCCESS;
}
...
int query_halt(void)
{
	...
	ret = os_thread_delete(query_thread);
	if (ret != WM_SUCCESS)
