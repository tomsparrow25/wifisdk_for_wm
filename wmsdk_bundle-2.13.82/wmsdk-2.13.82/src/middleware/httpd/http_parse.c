...
void httpd_parse_useragent(char *hdrline, httpd_useragent_t * agent)
{
	...
	if (strcmp(product, "Mozilla") == 0) {
		if (comment && strncmp(comment, MOZ_COMPATIBLE,
				       sizeof(MOZ_COMPATIBLE) - 1) == 0) {
				  ...
				  httpd_request_t *req_p, uint8_t *done)
{
	...
	if (strncasecmp(data_p, http_content_len,
			sizeof(http_content_len) - 1) == 0) {
		req_p->body_nbytes =
		...
		if (!first_double_quote) {
			httpd_w("If_None_Match has no double quote");
			return -WM_FAIL;
		}
	...
	} else if (strncasecmp(data_p, http_encoding,
			       sizeof(http_encoding) - 1) == 0) {
...
int httpd_parse_hdr_tags(httpd_request_t *req, int sock, char *buffer, int len)
{
	...
	while (TRUE) {
		...
		if (req_line_len == -WM_FAIL) {
			httpd_e("Could not read line from socket");
			return -WM_FAIL;
		}
...
#define TOKEN_LEN 10
	...
	if (err != WM_SUCCESS) {
		httpd_e("Unknown request type %s", data_p);
				  ...
				  char *val, unsigned val_len, short *found)
{
	...
	*found = 1;
	...
	if (i == val_len) {
		val[val_len] = 0;
		if ((*ptr != '&') && (*ptr != 0))
			httpd_w("Max val length exceeded.  "
				"Truncating value ");
	}
