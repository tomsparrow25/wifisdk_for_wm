/*
 * This file holds all power management code for WIFI
 * It is created to add mlan support to wmsdk
 */
...
void wifi_configure_null_pkt_interval(int null_pkt_interval)
{
	pmlan_adapter pmadapter = ((mlan_private *)
				   mlan_adap->priv[0])->adapter;
...
int wifi_configure_listen_interval(int time_in_ms)
{
	t_u16 bcon_period = 0;
	mlan_private *pmpriv = (mlan_private *) mlan_adap->priv[0];

	bcon_period = pmpriv->curr_bss_params.bss_descriptor.beacon_period;
	mlan_status ret = MLAN_STATUS_SUCCESS;
	int listen_interval = 0;
	if (time_in_ms < bcon_period
	    && time_in_ms >=  MIN_LISTEN_INTERVAL_IN_TU) {
		listen_interval = time_in_ms;
	} else if  (time_in_ms >
		    (bcon_period * MAX_LISTEN_INTERVAL_IN_BCON)
		    && time_in_ms <= MAX_LISTEN_INTERVAL_IN_TU) {
		listen_interval = time_in_ms;
	} else if (time_in_ms <=
		   (bcon_period * MAX_LISTEN_INTERVAL_IN_BCON)
		   && time_in_ms >= bcon_period) {
		listen_interval = time_in_ms / bcon_period;
	} else {
...
static int wifi_send_hs_cfg_cmd(t_u32 ipv4_addr,
...
int wifi_send_hs_cfg_cmd_uap(t_u32 ipv4_addr,
			 t_u16 action, t_u32 conditions)
{
	return wifi_send_hs_cfg_cmd(ipv4_addr,
				    action,
				    conditions,
				    MLAN_BSS_TYPE_UAP);
}
