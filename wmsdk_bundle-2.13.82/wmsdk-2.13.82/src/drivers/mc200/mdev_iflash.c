...
mdev_t *iflash_drv_open(const char *name, uint32_t flags)
{
	...
	if (mdev_p == NULL) {
		IFLASH_LOG("driver open called without registering device"
							" (%s)\n\r", name);
		return NULL;
	}
...
int iflash_drv_erase(mdev_t *dev, unsigned long start, unsigned long size)
{
	...
	if (FLASH_GetInterface() == FLASH_INTERFACE_FLASHC) {
		IFLASH_LOG("Cannot erase when Flash Controller is enabled\n\r");
		return -WM_FAIL;
	}

	ret = os_mutex_get(&iflash_mutex, OS_WAIT_FOREVER);
	if (ret == -WM_FAIL) {
		IFLASH_LOG("failed to get mutex\n\r");
		return ret;
	}
...
int iflash_drv_write(mdev_t *dev, uint8_t *buf, uint32_t len, uint32_t addr)
{
	...
	if (FLASH_GetInterface() == FLASH_INTERFACE_FLASHC) {
		IFLASH_LOG("Cannot write when Flash Controller is enabled\n\r");
		return -WM_FAIL;
	}
...
int iflash_drv_init(void)
{
	...
	if (ret == -WM_FAIL) {
		IFLASH_LOG("Failed to create mutex");
		return -WM_FAIL;
	}
