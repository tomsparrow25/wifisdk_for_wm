...
#define MC200_GPT_NUM 4

/* gpt mdev and synchronization handles */
static mdev_t mdev_gpt[MC200_GPT_NUM];
static os_mutex_t gpt_mutex[MC200_GPT_NUM];

/* gpt device specific names */
static const char *mdev_gpt_name[MC200_GPT_NUM] = {
	"MDEV_GPT0",
	"MDEV_GPT1",
	"MDEV_GPT2",
	"MDEV_GPT3"
...
static GPT_Config_Type gptdev_data[MC200_GPT_NUM];
...
int gpt_drv_init(GPT_ID_Type id)
{
	...
	ret = os_mutex_create(&gpt_mutex[id], mdev_gpt_name[id],
						OS_MUTEX_INHERIT);
	if (ret == -WM_FAIL) {
		GPT_LOG("Failed to create mutex\r\n");
		return -WM_FAIL;
	}
