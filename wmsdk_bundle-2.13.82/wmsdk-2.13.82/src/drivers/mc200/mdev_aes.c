...
int aes_drv_init()
{
	...
	if (status != WM_SUCCESS) {
		ADEBUG_E("AES hardware init failed");
		return -WM_FAIL;
	}
...
/*
 * len is in terms of uint32_t. i.e. If len = 1 then number of bytes is 4.
 *
 * Assumes AES_LOCK is taken.
 */
static void aes_operation(aes_t *aes, uint32_t *indata,
			  uint32_t *outdata, uint32_t len)
{
	uint32_t inputCnt = 0;
	...
	AES_Enable();
	while ((inputCnt < len) || (outputCnt < len)) {

		if ((inputCnt < len) &&
		    (!AES_GetStatus(AES_STATUS_INFIFO_FULL))) {

			AES_FeedData(indata[inputCnt]);
			inputCnt++;
		}

		if ((outputCnt < len) &&
		    (!AES_GetStatus(AES_STATUS_OUTFIFO_EMPTY))) {
			outdata[outputCnt] = AES_ReadData();
			outputCnt++;
		}
	}
		    ...
		    uint8_t *cipher, uint32_t sz)
{
	...
	LOCK_AES;
	aes_operation(aes, (uint32_t *) plain, (uint32_t *) cipher, sz / 4);
	UNLOCK_AES;
	return WM_SUCCESS;
}
		    ...
		    uint8_t *plain, uint32_t sz)
{
	...
	LOCK_AES;
	aes_operation(aes, (uint32_t *) cipher, (uint32_t *) plain, sz / 4);
	UNLOCK_AES;
	return WM_SUCCESS;
}
