...
/** The operation to change power state could not be perfomred*/
 ...
 * 	       flash descriptor.
 *
 * \return WLAN_ERROR_NONE if the WLAN Connection Manager service has
 ...
 *
 * \return WLAN_ERROR_NONE if the WLAN Connection Manager service has started
 *         successfully.  
 * \return WLAN_ERROR_PARAM if the \a cb pointer is NULL.  
 * \return WLAN_ERROR_ACTION if an internal error occurred.  
 ...
 *  micro-AP interface.
 *
 *  \return WLAN_ERROR_NONE if the WLAN Connection Manager service has been
 ...
 *          WLAN_DISCONNECTED,  \ref WLAN_ASSOCIATED or
 *          \ref WLAN_CONNECTED states.
 ...
 *  completed.
 *
 *  When connecting to a specific network, the event refers to the connection
 *  attempt to that network. When connecting to any network,
 *  the WLAN Connection Manager generates an event once it has succeeded
 *  in connecting to one of the known networks or once it has tried and
 *  failed to connect to every known network.
 *
 ...
 *
 *  \return WLAN_ERROR_NONE if a connection attempt was started successfully 
 *  \return WLAN_ERROR_STATE if the WLAN Connection Manager was not running.  
 *  \return WLAN_ERROR_PARAM if there are no known networks to connect to 
 *          or the network specified by \a name is not in the list
 *          of known networks or network \a name is NULL.
 *  \return WLAN_ERROR_ACTION if an internal error has occurred.
 */
 ...
 *
 * \return  WLAN_ERROR_NONE if successful
 ...
 *
 *  \return WLAN_ERROR_NONE if successful.
 *  \return WLAN_ERROR_STATE if in power save state.
 *  \return WLAN_ERROR_PARAM if \a name was NULL or the network \a
 ...
 *              copied.
 *
 *  \return WLAN_ERROR_NONE if the MAC address was copied.
 *  \return WLAN_ERROR_PARAM if \a dest is NULL.  
 */
 ...
 *
 *  \return WLAN_ERROR_NONE if successful.
 *  \return WLAN_ERROR_PARAM if \a network is NULL.
 *  \return WLAN_ERROR_STATE if the WLAN Connection Manager was
 ...
 *
 *  \returns WLAN_ERROR_NONE if successful.
 *  \return WLAN_ERROR_PARAM if \a network is NULL or \a index is out of range.
 */
 ...
 *
 *  \return WLAN_ERROR_NONE if successful.
 *  \return WLAN_ERROR_PARAM if \a network is NULL or \a name is NULL.
 */
 ...
 *
 *  \return WLAN_ERROR_NONE if successful. 
 *  \return WLAN_ERROR_PARAM if \a count is NULL.
 */
 ...
 *
 *  \return WLAN_ERROR_NONE if successful.
 *  \return WLAN_ERROR_PARAM if \a state is NULL 
 *  \return WLAN_ERROR_STATE if the WLAN Connection Manager was not running.
 */
 ...
 *
 *  \return WLAN_ERROR_NONE if successful.
 *  \return WLAN_ERROR_PARAM if \a cb is NULL,
 *  \return WLAN_ERROR_STATE if the WLAN Connection Manager was
 *           not running or not in the \ref WLAN_DISCONNECTED or \ref
 *           WLAN_CONNECTED states.
 *  \return WLAN_ERROR_ACTION if an internal error has occurred and
 ...
 *
 *  \return WLAN_ERROR_NONE if successful.
 *  \return WLAN_ERROR_PARAM if \a res is NULL
 *  \return WLAN_ERROR_STATE if the WLAN Connection Manager
 *          was not running
 *  \return WLAN_ERROR_ACTION if the scan result at \a
 ...
 *  
 *  \param [in]  time_in_milli_secs Time interval in ms, 
 *  		 -1 to disable
 *  \return      WLAN_ERROR_NONE or WLAN_ERROR_STATE
 */
int wlan_configure_listen_interval(int time_in_milli_secs);
 ...
 *
 * \return WLAN_ERROR_NONE if the call was successful.
 ...
 *
 * \param[out] version_str The buffer in which the wifi firmware
 * version extension string will be copied.
 */
 ...
 *
 * \return WLAN_ERROR_NONE if the call was successful.
 * \return WLAN_ERROR_PARAM if given parameters are not valid.
 ...
 *
 * \return WLAN_ERROR_NONE if successful.
 * \return WLAN_ERROR_PARAM if \a ps_mode was NULL.
 */
 ...
 *
 * \return WLAN_ERROR_NONE if successful.
 * \return WLAN_ERROR_ACTION if failed.
 */
 ...
 * 	      be a logical OR of the conditions in \ref wlan_wakeup_event_t. Typically
 * 	      devices would want to wake up on \ref WAKE_ON_UNICAST, \ref
 * 	      WAKE_ON_ARP_BROADCAST, \ref WAKE_ON_MGMT_FRAME and \ref WAKE_ON_MAC_EVENT.
 *
 */
