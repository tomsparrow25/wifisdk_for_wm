 ...
 */
static inline
int http_open_session_with_rcv_timeout(http_session_t *handle,
				       const char *hostname, int flags,
				       const tls_init_config_t *cfg,
				       int retry_cnt, unsigned int timeout)
{
	int rc = http_open_session(handle, hostname, flags, cfg, retry_cnt);
	if (WM_SUCCESS != rc)
		return rc;
	return http_setsockopt(*handle, SOL_SOCKET, SO_RCVTIMEO,
			     &timeout, sizeof(unsigned int));
}
