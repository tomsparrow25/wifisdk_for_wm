...
/*-----------------------------------------------------------*/
	...
	unsigned short usStackRemaining;
		...
		do
		{
			 ...
			 * 15th registers.
			 */
			sprintf( pcStatusString, ( char * ) "%s\t\t%c\t%u\t%u\t%u\t%x\t%x\r\n", pxNextTCB->pcTaskName, cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining, ( unsigned int ) pxNextTCB->uxTCBNumber, pxNextTCB->pxTopOfStack[13], pxNextTCB->pxTopOfStack[14] );
	...
	if (!listLIST_IS_EMPTY(pxList)) {
		...
		do {
			...
			if (pxTCB != pxTCB_first) {
				if (rem_size <= 0)
					break;
				snprintf(pcWriteBufferStart,
					 rem_size, "%s ", pxTCB->pcTaskName);
				buf_len = strlen(pcWriteBufferStart);
				rem_size -= buf_len;
