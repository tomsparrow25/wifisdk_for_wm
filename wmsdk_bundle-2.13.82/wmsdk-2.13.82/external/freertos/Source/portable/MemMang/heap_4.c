...
#define WMSDK_HEAP_LAST_ADDR WMSDK_HEAP_START_ADDR + WMSDK_HEAP_SIZE - 1
...
xBlockLink *pxLink;
 ...
 ASSERT( ( pv >= (void*)WMSDK_HEAP_START_ADDR ) ||
	 ( pv == NULL ) );
 ASSERT( ( pv <= (void*)WMSDK_HEAP_LAST_ADDR ) ||
	 ( pv == NULL ) );
...
int vHeapSelfTest( int trace )
{
	{ // find out total size of heap
		...
		if( trace ) {
			...
			DTRACE( "%s%11s%11s%11s%11s\n\r",
					"TAG", "Next Free", "Prev Block", "Block Size", "Block Type");
#endif /* DEBUG_HEAP_EXTRA */
			DTRACE( "HST%11x%11x%11d%11c\n\r",
				       xStart.pxNextFreeBlock, xStart.pxPrev,  
					xStart.xBlockSize, 'X' );
		}
	        while( 1 ) {
			if( trace ) {
