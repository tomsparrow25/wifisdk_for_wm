...
#define  EHCI_PORTSCX_OVERCURRENT_CHANGE           (0x00000020)
#define  EHCI_PORTSCX_OVERCURRENT_ACTIVE           (0x00000010)
...
#define  EHCI_ITD_STATUS_ACTIVE            (0x80000000)
#define  EHCI_ITD_STATUS_DATA_BUFFER_ERR   (0x40000000)
#define  EHCI_ITD_STATUS_BABBLE_ERROR      (0x20000000)
#define  EHCI_ITD_STATUS_TRANSACTION_ERR   (0x10000000)
...
#define  VUSBHS_OTGSC_VD        (0x00000001)	/* Vbus discharge */
	...
	typedef uint32_t USB_REGISTER, *USB_REGISTER_PTR;
...
/* The VUSB register structure */
	typedef USB_Uncached struct {
		union {
			...
			struct {
				...
				USB_REGISTER RESERVED0;
				USB_REGISTER BURST_SIZE;	/* Burst Size */
				USB_REGISTER RESERVED1[7];
			...
			struct {
				...
				USB_REGISTER BURST_SIZE;
				USB_REGISTER TX_FILL_TUNING;
				USB_REGISTER RESERVED0[6];
				USB_REGISTER CONFIG_FLAG;	/* Configured Flag register */
				USB_REGISTER PORTSCX[VUSBHS_MAX_PORTS];	/* Port Status/Control x, x = 1..8 */
	...
	typedef USB_Uncached struct {
		uint32_t MAX_PKT_LENGTH;	/* Bits 16..26 Bit 15 is Interrupt
					 ** On Setup
					 */
		uint32_t CURR_DTD_PTR;	/* Current dTD Pointer */
	...
	typedef USB_Uncached struct ep_tr_struct {
		...
		uint32_t SIZE_IOC_STS;	/* total bytes (16-30),
					 ** IOC (15), Status (0-7)
					 */
		uint32_t BUFF_PTR0;	/* Buffer void * Page 0 */
		uint32_t BUFF_PTR1;	/* Buffer void * Page 1 */
		uint32_t BUFF_PTR2;	/* Buffer void * Page 2 */
		uint32_t BUFF_PTR3;	/* Buffer void * Page 3 */
		uint32_t BUFF_PTR4;	/* Buffer void * Page 4 */
	...
	typedef USB_Uncached struct {
		...
		uint32_t TR_STATUS_CTL_LIST[8];	/* bits 31-28: Status,
						 ...
						 */
		uint32_t BUFFER_PAGE_PTR_LIST[7];	/* bits 31-12 4K aligned void *
						 ** to physical memory
	...
	typedef USB_Uncached struct {
		...
		uint32_t TRANSFER_STATE;	/* bit 31: int on complete
					 ...
					 ** bits 7-0: status
					 */
		uint32_t BUFFER_PTR_0;	/* bits 31-12: 4K aligned void *
					 ** to physical memory
					 ** bits 11-0: Current offset
					 */
		uint32_t BUFFER_PTR_1;	/* bits 31-12: 4K aligned void *
					 ** to physical memory
					 ** bits 11-5 reserved
					 ** bits 4-3 tr position
					 ** bits 2-0 tr count
					 */
		uint32_t BACK_LINK_PTR;	/* bits 31-5 back void * points to sITD
					 ** bits 4-1: reserved
					 ** bit 0: terminate
					 */
		SCRATCH_STRUCT_PTR SCRATCH_PTR;
