/* internal.c
 *
 * Copyright (C) 2006-2012 Sawtooth Consulting Ltd.  All rights reserved.
 *
...
/* wmsdk: added to avoid memory usage */
...
static int DoCertificate(CYASSL* ssl, byte* input, word32* inOutIdx)
{
    ...
    TRACE("Total certificates in chain : %d\n\r", totalCerts);

    count = totalCerts;

    TRACE("Decoding the certificates ...\n\r");
	...
	TRACE("Parse certificate- Length: %d\n\r", myCert.length);
...
static int GetInputData(CYASSL *ssl, word32 size)
{
    ...
    do {
	    TRACE("Blocking for data from network ... expecting %d bytes\n\r", inSz);
