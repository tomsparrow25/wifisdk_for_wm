...
#ifdef DEBUG_CYSSL
unsigned wmprintf(const char *format, ...);
#else
#define wmprintf(...)
#endif
...
CYASSL* CyaSSL_new(CYASSL_CTX* ctx)
{
    ...
    if (ssl) {
	    ...
	    wmprintf("Error: InitSSL failed\n\r");
            FreeSSL(ssl);
            ssl = 0;
       }
    }  else {
        wmprintf("Error: Unable to allocate SSL object\n\r");
    }
...
{
       wmprintf("In exit ... 0x%x\n", ret_val);
       while(1);
}

static void print_current_tcb()
{
	/* dummy function */
}

void print_current_tcb();
signed char * get_current_taskname();
void _sbrk()
{
       wmprintf("In sbrk ... Task: %s  \n", get_current_taskname());
       print_current_tcb();
       while(1);
}
...
int _gettimeofday()
{
       wmprintf("In _gettimeofday ... Task: %s  \n", get_current_taskname());
       return 0;
       while(1);
}
