...
/* added as we have no function gmtime */
static struct tm tm_local;

#define XTIME(tl) wmtime_time_get_posix()
#define XGMTIME(c) gmtime_r((c),&tm_local)
#define XVALIDATE_DATE(d, f, t) ValidateDate((d), (f), (t))
...
int ValidateDate(const byte* date, byte format, int dateType)
{
    ...
    /* wmsdk: kept for certificate related debugging */
#if 0
    wmprintf("Certificate time:\n\r");
    dump_tm(&certTime);
    wmprintf("Local time:\n\r");
    dump_tm(localTime);
#endif /* 0 */
...
int DecodeToKey(DecodedCert* cert, int verify)
{
    ...
    TRACE("%s: Certificate: \n\rSubject: %s\n\rIssuer: %s\n\r\n\r", 
         __FUNCTION__, cert->subject, cert->issuer);
...
void DumpHex(const byte *arr, int size)
{
       int cnt;
...
void DumpASCII(const byte *arr, int size)
{
       int cnt;
                       TRACE("\n\r");
       for(cnt = 0; cnt < size; cnt++) {
...
void DumpCertificate(const DecodedCert* cert)
{
#if 0
       TRACE("\n\r********** CERTIFICATE *********\n\r");
       TRACE("Public key:");
       DumpHex(cert->publicKey, cert->pubKeySize);
       TRACE("Public key stored: %d\n\r", cert->pubKeyStored);
       TRACE("Offset to start of cert: %x\n\r", cert->certBegin);
       TRACE("Offset to start of signature: %x\n\r", cert->sigIndex);
       TRACE("Length of signature: %x\n\r", cert->sigLength);
       TRACE("Sum of algorithm object id: %x\n\r", cert->signatureOID);
       TRACE("Sum of key algo  object id: %x\n\r", cert->keyOID); 
       TRACE("Hash of all Names (subjectHash):\n\r");
       DumpHex(cert->subjectHash, SHA_SIZE);
       TRACE("Hash of all Names (issuerHash):\n\r");
       DumpHex(cert->issuerHash, SHA_SIZE);
       TRACE("Not owned, points into raw cert (signature): %x\n\r", cert->signature);
       TRACE("CommonName: %s\n\r", cert->subjectCN);
       TRACE("subjectCNLen: %d\n\r", cert->subjectCNLen);
       TRACE("Full name including common name (issuer): %s\n\r", cert->issuer); 
       TRACE("Full name including common name (subject): %s\n\r", cert->subject); 
       TRACE("Default to yes, but could be off (verify): %d\n\r", cert->verify);
       TRACE("Byte buffer holder cert, NOT owner: %x\n\r", cert->source);
       TRACE("Current offset into buffer: %x\n\r", cert->srcIdx);
       TRACE("For user memory overrides: %x\n\r", cert->heap);
       TRACE("Raw serial number: \n\r");
       DumpHex(cert->serial, EXTERNAL_SERIAL_SIZE); 
       TRACE("Raw serial bytes stored: %d\n\r", cert->serialSz);
#ifdef CYASSL_CERT_GEN
#error "Enable this dump also"
...
int ParseCertRelative(DecodedCert* cert, int type, int verify,  void* cm)
{
    ...
    if (badDate != 0) {
	    /*
	     * The board which we are using for development does not have a
	     * battery backed up RTC. Hence, the date is reset always after
	     * board reboot. So, the correct date needs to be set manually
	     * every time. Without that things will fail.
	     */
           TRACE("BAD-DATE\n\r");
           return badDate;
    }
