...
/*
 * Following functions are defined in wmsdk. They are functions which
 * implement AES hardware accelerations. Adding only prototypes here.
 * We have done this because cyassl being below WMSDK layer, cannot
 * include WMSDK header files which declare these functions.
 */


/** 
 * Set the AES encryption/decryption parameters
 * 
 * @par
 * This function sets the Saratoga hardware accelerator AES parameters.
 *
 * @note \ref aes_setkey sets the hardware mode to HW_AES_MODE_CBC. Thus,
 * if you do not want CBC mode please set the desired mode through 
 * \ref aes_setmode.
 *
 * @param[in,out] aes Preallocted object of type Aes. Will be filled by callee
 * @param[in] userKey AES encryption/decryption key.
 * @param[in] key_len AES encryption/decryption key length.
 * @param[in] IV AES encryption/decryption initialization vector.
 * @param[in] dir Either AES_ENCRYPTION or AES_DECRYPTION
 *
 * @return Result of the operation.
 */
int cyassl_wrapper_aes_setkey(Aes *aes, const byte *userKey,
			      word32 key_len, const byte *IV, int dir,
 ...
 * 
 * @par
 * This function encrypts the given plain text into cipher text using
 * Saratoga AES hardware accelerator.
 *
 * @note. Please ensure that the size of the cipher buffer is atleast equal
 * to sz
 *
 * @param[in,out] aes As returned from earlier call to \ref aes_setkey
 * @param[in] plain Plain text to be encrypted
 * @param[out] cipher Encrypted data
 * @param[in] sz size of plain text in bytes. Please ensure that the size
 * of the cipher buffer is atleast equal to sz
 *
 * @return Result of the operation.
 */
int cyassl_wrapper_aes_encrypt(Aes *aes, const byte *plain,
			       byte *cipher, word32 sz);

/** 
 * Decrypt the cipher text into plain text. 
 * 
 * @par
 * This function decrypts the given cipher text into plain text using
 * Saratoga AES hardware accelerator.
 *
 * @note. Please ensure that the size of the plain buffer is atleast equal
 * to sz
 *
 * @param[in,out] aes As returned from earlier call to \ref aes_setkey
 * @param[in] cipher Encrypted data
 * @param[out] plain Plain text to be encrypted
 * @param[in] sz size of cipher text in bytes. Please ensure that the size
 * of the plain buffer is atleast equal to sz
 *
 * @return Result of the operation.
 */
int cyassl_wrapper_aes_decrypt(Aes *aes, const byte *cipher,
			       byte *plain, word32 sz);


int cyassl_wrapper_aes_ccm_setkey(Aes *aes, const byte *userKey,
				  word32 key_len);
void cyassl_wrapper_aes_ccm_encrypt(Aes* aes, byte* out,
				    const byte* in, word32 inSz,
				    const byte* nonce, word32 nonceSz,
				    byte* authTag, word32 authTagSz,
				    const byte* authIn, word32 authInSz);
int  cyassl_wrapper_aes_ccm_decrypt(Aes* aes, byte* out,
				    const byte* in, word32 inSz,
				    const byte* nonce, word32 nonceSz,
...
{
#ifdef MC200_AES_HW_ACCL
	return cyassl_wrapper_aes_setkey(aes, userKey, keylen, iv, dir, 
				  HW_AES_MODE_CBC);
...
{
#ifdef MC200_AES_HW_ACCL
	cyassl_wrapper_aes_encrypt(aes, in, out, sz);
#else /* ! MC200_AES_HW_ACCL */
...
{
#ifdef MC200_AES_HW_ACCL
	cyassl_wrapper_aes_decrypt(aes, in, out, sz);
#else /* ! MC200_AES_HW_ACCL */
    word32 blocks = sz / AES_BLOCK_SIZE;
...
{
#ifdef MC200_AES_HW_ACCL
    cyassl_wrapper_aes_ccm_setkey(aes, key, keySz);
#else
...
{
#ifdef MC200_AES_HW_ACCL
    cyassl_wrapper_aes_ccm_encrypt(aes, out, in, inSz, nonce, nonceSz,
...
{
#ifdef MC200_AES_HW_ACCL
	return cyassl_wrapper_aes_ccm_decrypt(aes, out, in, inSz,
		nonce, nonceSz, authTag, authTagSz, authIn, authInSz);
#else
    byte A[AES_BLOCK_SIZE];
    byte B[AES_BLOCK_SIZE];
    byte* o;
